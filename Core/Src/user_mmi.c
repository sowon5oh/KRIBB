...
//void MMI_AdcResultSener(MeasM1Result_t *p_result, uint8_t result_num);
...
void MMI_M1ResultSender(MeasM1Result_t *p_result, uint8_t result_num) {
	...
	MMI_Sender(CMD_1_DEV_SEND_RESULT, CMD_2_RESULT_ADC, CMD_3_M1_RESULT,
			(uint8_t*) &result_buff[0],
...
void MMI_M3ResultSender(MeasM3Result_t *p_result) {
	...
	MMI_Sender(CMD_1_DEV_SEND_RESULT, CMD_2_RESULT_ADC, CMD_3_M3_RESULT,
			(uint8_t*) &result_buff[0], LED_MEAS_M3_RESULT_DATA_SIZE);
}
...
static void _procDevAdcSet(uint8_t *p_arr) {
	...
	uint8_t len = (p_arr[MMI_PROTOCOL_LEN_IDX] << 8)
			| p_arr[MMI_PROTOCOL_LEN_IDX + 1];
	uint8_t data[12];
	...
	case CMD_2_LED_SET:
		if (cmd_3 == CMD_3_LED_SET_W) {
			...
			for (int i = 0; i < cnt; i++) {
				...
				schedules[i].led_data = (data[i * 3 + 1] << 8)
						| data[i * 3 + 2];
			}
			LED_TASK_SetSchedule(schedules, cnt);
			...
			//LED_TASK_SetAdc((MeasAvrReq_t *)&data[0]);
		}
		break;
	case CMD_2_ADC_SET:
		if (cmd_3 == CMD_3_ADC_CFG_W) {
			...
			LED_TASK_CFG_Change(&meas_cfg);
			ADC_CFG_Change(&meas_cfg);

			if (meas_cfg.ch == 0) //ch1
					{
				HAL_GPIO_WritePin(M_SEL_EN_GPIO_Port, M_SEL_EN_Pin,
						GPIO_PIN_SET);
				HAL_GPIO_WritePin(M_SEL_A0_GPIO_Port, M_SEL_A0_Pin,
						GPIO_PIN_RESET);
				HAL_GPIO_WritePin(M_SEL_A1_GPIO_Port, M_SEL_A1_Pin,
						GPIO_PIN_RESET);
				meas_cfg.Mux_ch = 1;
