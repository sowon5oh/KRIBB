...
//    /* Reset */
//    SYS_VERIFY_SUCCESS(_ads130b04SendCmd(ADS130B04_CMD_ID_RESET));

    /* Read ID */
    SYS_VERIFY_SUCCESS(_ads130b04ReadRegister(ADS130B04_REG_ADDR_ID, 1, &read_data));
    SYS_LOG_DEBUG("ADC ID: %04X", read_data); //5404 - 21508

    SYS_VERIFY_SUCCESS(_ads130b04ReadRegister(ADS130B04_REG_ADDR_STATUS, 1, &read_data));
    SYS_LOG_DEBUG("ADC STATUS: %04X", read_data); //0500 - 1280

    SYS_VERIFY_SUCCESS(_ads130b04ReadRegister(ADS130B04_REG_ADDR_MODE, 1, &read_data));
    SYS_LOG_DEBUG("ADC MODE: %04X", read_data); //0510 - 1296

    SYS_VERIFY_SUCCESS(_ads130b04ReadRegister(ADS130B04_REG_ADDR_CLOCK, 1, &read_data));
    SYS_LOG_DEBUG("ADC CLOCK: %04X", read_data); //0F8E - 3970

    /* Standby */
    SYS_VERIFY_SUCCESS(_ads130b04SendCmd(ADS130B04_CMD_ID_STANDBY));
...
HAL_StatusTypeDef DRV_ADS130B04_Start(void) {
    /* Wake Up */
    return _ads130b04SendCmd(ADS130B04_CMD_ID_WAKEUP);
}
...
//    SYS_VERIFY_SUCCESS(_ads130b04SetClockCfg());
}
...
void DRV_ADS130B04_MeasStop(void) {
    ...
    SYS_VERIFY_SUCCESS_VOID(_ads130b04SetClockCfg());
}
...
HAL_StatusTypeDef DRV_ADS130B04_ReqAvr(ads130b04Data_t *p_req_info, MeasAvrResultCb_t cb_fn) {
    ...
    SYS_VERIFY_PARAM_NOT_NULL(p_req_info);
    ...
    /* Get first data to init FIFO for data ready signal */
...
HAL_StatusTypeDef _ads130b04SendCmd(ads130b04CmdId_t cmd) {
    ...
    switch (cmd) {
        ...
        default:
            SYS_LOG_ERR("Not supported command %04X", send_cmd);
            return HAL_ERROR;
    }
    ...
    SYS_VERIFY_SUCCESS(_ads130b04TransmitReceive(&tx_buff[0], &rx_buff[0], 6));
...
HAL_StatusTypeDef _ads130b04GetData(bool first_read) {
    ...
    SYS_VERIFY_SUCCESS(_ads130b04TransmitReceive(&tx_buff[0], &rx_buff[0], 6)); //3
...
HAL_StatusTypeDef _ads130b04SetClockCfg(void) {
    ...
    SYS_VERIFY_SUCCESS(_ads130b04WriteRegister(ADS130B04_REG_ADDR_CLOCK, send_cfg));
...
HAL_StatusTypeDef _ads130b04SetGainCfg(void) {
    ...
    SYS_VERIFY_SUCCESS(_ads130b04WriteRegister(ADS130B04_REG_ADDR_GAIN, send_cfg));
...
HAL_StatusTypeDef _ads130b04SetChMuxCfg(void) {
    ...
    for (uint8_t idx = 0; idx < 4; idx++) { //5
        ...
        SYS_VERIFY_SUCCESS(_ads130b04WriteRegister(ADS130B04_REG_ADDR_CH0_CFG + 5 * idx, send_cfg));
...
HAL_StatusTypeDef _ads130b04WriteRegister(uint16_t reg, uint16_t data) {
    ...
    SYS_VERIFY_SUCCESS(_ads130b04Transmit(&send_data[0], 3));
    SYS_VERIFY_SUCCESS(_ads130b04Receive(&resp_data, 1));
    ...
    SYS_VERIFY_SUCCESS(_ads130b04ReadRegister(reg, 1, &save_data));
...
HAL_StatusTypeDef _ads130b04ReadRegister(uint16_t reg, uint8_t len, uint16_t *p_data) {
    ...
    SYS_VERIFY_SUCCESS(_ads130b04Transmit(&send_data[0], 2));
    SYS_VERIFY_SUCCESS(_ads130b04Receive(&read_data[0], len));
