...
typedef enum {
    ADC_STATE_STANDBY = 0,
    ADC_STATE_CONTINOUS, /* ADC data are generated constantly at the rate of fDATA = fMOD / OSR. New data are indicated by a DRDY falling edge at this rate */
    ADC_STATE_GLOBAL_CHOP, /* enabled by setting the GC_EN bit in the GLOBAL_CHOP_CFG register */
} adcStateMode_t;

typedef enum {
    ADC_CLOCK_INTERNAL_OSC = 0,
    ADC_CLOCK_EXTERNAL_CLOCK,
} adcClockSel_t;
...
typedef enum {
    ADC_WORD_SIZE_16_BIT = 0,
    ADC_WORD_SIZE_24_BIT, /* Default */
...
/* Private variables ---------------------------------------------------------*/
adcContext_t adc_context = { .adc_read = false, .adc_num = 0, .adc_cnt = 0, .adc_wait_time = 0, .word_size.byte_num = ADC_SPI_WORD_BYTE_NUM_24BIT };
